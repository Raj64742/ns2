## WARNING!  THIS CODE IS VERY PREMATURE, AND TRANSIENT, AND
...
double p_to_b(double p, double rtt, double tzero, int psize) {
...
double b_to_p(double b, double rtt, double tzero, int psize) {
  double p, pi, bres;
  int ctr=0;
...
TfrmSinkAgent::TfrmSinkAgent() : Agent(PT_MESSAGE), rate_(0.0), 
  last_arrival_(0.0),
  last_nack_(0.0), 
...
void TfrmSinkAgent::recv(Packet *pkt, Handler *)
{
  struct payload *pld;
  received_++;
  pld = (struct payload*)pkt->accessdata();
#ifdef DEBUG
...
void TfrmSinkAgent::sendpkt()
{
  ...
  if (pveclen_<sample) {
	...
	if ((last_loss - tsvec_[ix]) > rtt_) {
	  //only count losses if they're separate congestion events
	  lost++;
	  last_loss = tsvec_[ix];
	}
      }

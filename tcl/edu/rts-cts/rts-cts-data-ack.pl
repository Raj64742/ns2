...
while (($line) && ($tp ==0)) { 
	if ($line =~ /-i 3 -n green/) {
		...
		print Destination 'v -t 0.001 -e sim_annotation 0.001 2 carrier sense only provides information about collisions at the sender,not at ', "\n";
		print Destination 'v -t 0.002 -e sim_annotation 0.002 3 the receiver. This leads to problems like hidden terminal where two sender ', "\n";
		print Destination 'v -t 0.003 -e sim_annotation 0.003 4 nodes out of range of each other transmit packets at the same time, ', "\n";
		print Destination 'v -t 0.004 -e sim_annotation 0.004 5 to the same receiver, resulting in  collisions at the receiver. To solve ', "\n";
 		print Destination 'v -t 0.005 -e sim_annotation 0.005 6 such problems, control packets, RTS & CTS are used. When a node wants to ', "\n";
		print Destination 'v -t 0.006 -e sim_annotation 0.006 7 transmit packets, it first sends a RTS packet to the receiver.All nodes within ', "\n";
		print Destination 'v -t 0.007 -e sim_annotation 0.007 8 the sender\'s range receive this RTS packet. Every node hearing this RTS packet ', "\n";
		print Destination 'v -t 0.008 -e sim_annotation 0.008 9 will defer transmission. If receiver is not receiving data from any other node, ', "\n";
		print Destination 'v -t 0.009 -e sim_annotation 0.009 10 it responds with a CTS packet.This packet is again received by every node ', "\n";
		print Destination 'v -t 0.01 -e sim_annotation 0.01 11 within range of receiver. So all nodes in this range defer transmission. ', "\n";
		print Destination 'v -t 0.02 -e sim_annotation 0.02 12 Sender node sends data packet after receiving CTS. If sender node does not ', "\n";
		print Destination 'v -t 0.03 -e sim_annotation 0.03 13 hear the CTS, it will time-out and schedule retransmission of RTS. On ', "\n";
		print Destination 'v -t 0.04 -e sim_annotation 0.04 14 receiving data packet, receiver sends an ACK to sender. RTS and CTS packets ', "\n";
		print Destination 'v -t 0.05 -e sim_annotation 0.05 15 are very short, hence their collision is very less lilkely. If 2 RTS packets ', "\n";
		print Destination 'v -t 0.06 -e sim_annotation 0.06 16 from sender nodes collide at the receiver, sender nodes will not receive a ', "\n";
		print Destination 'v -t 0.07 -e sim_annotation 0.07 17 CTS packet and the nodes, as a result will schedule retransmission of RTS. ', "\n";
		print Destination 'v -t 0.10000000 -e set_rate_ext 0.550ms 1', "\n";
		$tp = 1;
	}
	else
	{
...
while($line) {
	...
	if ($line =~ /^M/)
	{
		...
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' Nodes 0 and 2 are in range of each other', "\n";
		$i ++;
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' and hence can hear the RTS packet', "\n";
		$i ++;
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' of the other node. After hearing RTS packet', "\n";
		$i ++;
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' node backs off', "\n";
		$i ++;
		...
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' Here nodes 0 and 2 are out of range of each other', "\n";
		$i ++;
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' hence they donot detect carrier and send RTS packets', "\n";
		$i ++;
		print Destination 'v -t ', @fields[1], ' -e sim_annotation  ', @fields[1], ' ' , $i, ' at the same time', "\n";
		$i ++;
		...
		$duration = @fields[1] + 0.005;
		print Destination 'v -t ', $duration, ' -e sim_annotation ', $duration ,' ', $i, ' Node ', @fields[2] , ' sending RTS to Node 1 ', "\n";
		print Destination 'n -t ', $duration, ' -s ', @fields[2], ' -S COLOR -c purple -o red -i purple -I red', "\n";
		print Destination 'n -t ', $duration, ' -s ', @fields[2], ' -S DLABEL -l "Sending RTS packet" -L ""', "\n";
		print Destination '+ -t ', $duration, ' -s ', @fields[2], ' -d -1 -p message -e 500 -a 7', "\n";
		print Destination '- -t ', $duration, ' -s ', @fields[2], ' -d -1 -p message -e 500 -a 7', "\n";
		...
		$temp = $new_duration;
		if ($t == 1) {
			print Destination 'v -t ', $new_duration, ' -e sim_annotation ', $new_duration, ' ', $i, ' Node ', $other_node, ' sending RTS to Node 1 ', "\n";
		$i ++;
		...
		print Destination 'v -t ', $jitter, ' -e sim_annotation ', $jitter ,' ', $i, ' Since nodes 0 \,2 and 1 are in range of each other ', "\n";
		$i++;
		print Destination 'v -t ', $jitter, ' -e sim_annotation ', $jitter ,' ', $i, ' they can back off after hearing CTS packets  ', "\n";
		$i++;
		print Destination 'v -t ', $jitter, ' -e sim_annotation ', $jitter ,' ', $i, ' from another node  ', "\n";
		$i++;
		...
		print Destination 'v -t ', $duration, ' -e sim_annotation ', $duration ,' ', $i, ' Node ', $other_node , ' sending RTS to Node ', @fields[2],  "\n";

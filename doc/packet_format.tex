...
\begin{enumerate}\itemsep0pt
  ...
  \item edit \nsf{tcl/lib/ns-packet.tcl} to enable new packet format
        ...
        Alternatively, one may call \code{create-packet-header} in
        one's own code (see \nsf{tcl/lib/ns-packet.tcl} for example
...
\code{off_ip_}.
Notice that here the \code{off_\tup{hdrname}_} stuff is only provided
...
\code{$pktHdrManager allochdr}\\
This is another internal procedure of Class PacketHeaderManager that keeps track
of a variable called \code{hdrlen_} as new packet-headers are enabled. It also

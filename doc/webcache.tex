...
This creates an instance of an Http server at the specified <s-node>. An instance
of the simulator <sim> nedds to be passed as an argument as well.
...
\code{$client set-page-generator <pgp>\\
...
The cache is also configured to log its events in a log-file.

\code{$client connect <cache>\\
$cache connect <server>}\\
Once the client, cache, and server are configures, they need to be connected.

\code{$client start-session <cache> <server>}\\
This starts sending request for a random page from the client to the <server>
via <cache>.

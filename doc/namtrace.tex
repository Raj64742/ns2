...
This gives nam the size of the layout for the wireless world.  The -x value is the width and -y is height. For more information look at the file/animator.tcl under the procedure infer-network-model.
...
"-S" gives the node state transition.\\
The possible state transition values are: 
\begin{itemize}
...
\begin{program}
f -t 0.00000000000000000 -s 1 -n C1\_ -a srm(1) -v 2.25000 -T v
\end{program}
...
\begin{description}
\item[Annotation] This event is represented by event type {\tt v}. 
  It is used for generic annotation:
\begin{program}
v -t <time> -e <TCL script string>  
\end{program}
  ...
  The order of flags and the string is important.

  Here is an example of this event:
\begin{program}
v -t 4 -e sim\_annotation 4 3 node 0 added one mark
\end{program}
  This line calls a special tcl function {\tt sim\_annotation} in
  nam, which inserts the given string \code{node 0 added one mark}
  into nam's annotation pane.  Look at ''Animator instproc sim\_annotation { time seq args }'' in tcl/annotation.tcl for implementation details.
\end{description}

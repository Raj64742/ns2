...
\begin{program}
	...
	set group [{\bfseries{}Node allocaddr}]   \; allocate a mulicast address;
...
Currently, it only traces register, join, prune, and data packets.
\begin{program}
...
it invokes \proc[]{Node::join-group}, 
which in turn calls
\proc[]{McastProtoArbiter::join-group} for the particular instance
...
deletes the receiver agents from the local \code{Agents_} list,
and invokes \proc[]{McastProtoArbiter::leave-group}.
...
then it will invoke \proc[]{new-group} for the classifer,
...
\proc[]{add-rep} creates a slot in the classifer
...
\proc[]{is-active} & returns true if the replicator has atleast one active slot.\\
\proc[oif]{exists} & returns true if the slot pointing to the specified oif is active.\\
\proc[]{change-iface} & modified the iif entry for the particular replicator.\\
\end{alist}
...
\begin{alist}
\proc[]{reset-mroutes} & resets all mutlicast forwarding entries.\\
\proc[]{compute-mroutes} & (re)computes all multicast forwarding entries.\\
\proc[]{compute-tree} & computes a multicast tree for one source to reach 
		all the receivers in a specific group.\\
\proc[]{compute-branch} & is executed when a receiver joins a multicast group.
...
\begin{alist}
\proc[]{join-group} & sends graft messages upstream if \tup{S,G} does not
	contain any active outgoing slots (\ie, no downstream receivers).\\
\proc[]{leave-group} & does not do anything.\\
\proc[]{handle-cache-miss} & creates \tup{S,G} with only
...
\proc[]{recv-graft} & cancels any existing prune timer, and

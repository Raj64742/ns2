...
\begin{longtable}{rl}
  ...
  UDP & a basic UDP agent\\
	\\
  CBR & a UDP-like agent with sequence numbers and a send timer \\
...
the \code{Agent/UDP} base class.
For protocol endpoints wishing to use a connectionless
datagram-oriented transport layer (like UDP) with sequencing and periodic
send events, the \code{CBR_Agent} base class is likely to be appropriate.
